...
export function createMockRunner(events: { broadcast: (m: ServerMsg) => void; log: (l: string) => void }) {
  ...
  const reqs = new Map<string, OwnerRequest>([
    ['REQ-mock-1', {
      ...
      valid: true, invalidReason: null, warn: null, options: null, chosenOption: null,
    ...
    ['REQ-mock-2', {
      ...
      options: [
        'Yes — textbook scans allowed with full citations',
        'No — primary literature and own derivations only',
        'Case-by-case — each scan needs its own owner request',
      ],
      chosenOption: null,
