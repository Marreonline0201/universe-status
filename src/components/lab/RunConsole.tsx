...
export function RunConsole({ office, requestId }: { office: OfficeSocket; requestId: string | null }) {
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(0,180,255,0.15)', display: 'flex', flexDirection: 'column', height: collapsed ? 28 : 220, flexShrink: 0 }}>
      ...
      >
        <span style={{ fontSize: 9, letterSpacing: 2, color: 'rgba(0,180,255,0.5)' }}>{collapsed ? '▸' : '▾'} LIVE CONSOLE</span>
        {req && (
          <>
            <span style={{ fontSize: 9, color: STATUS_COLORS[req.status] ?? '#8a97b8', fontWeight: 700, letterSpacing: 1 }}>
            ...
            <span style={{ fontSize: 9, color: '#8a97b8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{req.title}</span>
          </>
        )}
        <span style={{ marginLeft: 'auto', fontSize: 8, color: '#3a4157' }}>{lines.length} lines</span>
        {!stuck && !collapsed && (
          <span
            onClick={(e) => { e.stopPropagation(); setStuck(true) }}
            style={{ fontSize: 8, color: '#00d4ff', border: '1px solid #00d4ff55', borderRadius: 3, padding: '1px 6px', cursor: 'pointer' }}
          >▼ LATEST</span>
        )}
      </div>
      {!collapsed && (
        <div
          ...
          style={{ flex: 1, overflowY: 'auto', background: '#05070f', padding: '6px 10px', fontSize: 10, lineHeight: 1.5 }}
        >
          {lines.length === 0 && (
          ...
          {req && (req.status === 'done' || req.status === 'failed') && (
            <div style={{ marginTop: 4, color: req.status === 'done' ? '#00ff88' : '#ff4444', fontSize: 9, fontWeight: 700 }}>
